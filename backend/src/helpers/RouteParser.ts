...
function getResponses(comment: string[]): Array<string> {
	if (comment.indexOf('parameters:') != -1) {
		const lastValue = comment.indexOf('parameters:');
		const firstValue = comment.indexOf('responses:');
	...
	for (const filePath of endpoints.map(i => i.path)) {
		...
		if (jsdocAnnotations.length) {
			for (const annotation of jsdocAnnotations) {
				...
				data.responses = getResponses(newComment);
